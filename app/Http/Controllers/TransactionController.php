...
class TransactionController extends Controller
{
    ...
    {        
        ...
        $account_sales_id = AccountingConfig::where('name', 'sales')->first()->accounting_id;
        $account_partners_id = AccountingConfig::where('name', 'partners')->first()->accounting_id;
        ...
        try {
            ...
            foreach ($request->selected_partner as $key => $selected) {
                ...
                foreach ($request->partner_id as $key2 => $value2) {
                    ...
                    if ($request->partner_id[$key2] == $request->selected_partner[$key]) {
                        ...
                        //actualiza las cuentas del voucher que están en la transacción actual
            ...
            foreach ($this->transactions_list as $transaction) {
                ...
                $journal = Journal::create([
                    'number' => $number,
                    'date' => $transaction->date,
                    'refence' => "Transacción lotes: " .  $transaction->number . " - Socio: " . $partner->name,
                    'company_id' => session('company')->id,
                    'type' => Journal::AUTO,
                    'journable_id' => $trans->id,
                    'journable_type' => Transaction::class
                ]);
                
                JournalDetail::create([
                    'journal_id' => $journal->id,
                    'accounting_id' => $account_partners_id,
                    'debit_value' => $transaction->total,
                    'credit_value' => 0
                ]);
               
                JournalDetail::create([
                    'journal_id' => $journal->id,
                    'accounting_id' => $account_sales_id,
                    'debit_value' => 0,
                    'credit_value' => $transaction->total
                ]);
            }     
